...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className='Dashboard'>
      <div style={{border: '1px solid black',width:'550px',display:'flex',flexDirection:'column', alignItems:'center', justifyContent:'center' ,padding:'20px'}}>
      ...
      <div>
        <input
          ...
          value={newTodoTitle}
          className='input'
          placeholder='Todo New Tilte'
          onChange={(e) => setNewTodoTitle(e.target.value)}
        />
        <button onClick={handleAddTodo} className='Add__todo'>Add Todo</button>
      </div>
        {user?.todos.map((todo) => (
          <li key={todo.id} style={{fontSize:"25px", fontWeight:'bold' ,margin:'20px'}}>
            {todo.title}
            <button className='Remove__subTodo' onClick={() => handleRemoveTodo(todo.id)}>Remove</button>
            <button className='Add__subTodo' onClick={() => handleSelectTodo(todo.id)}>Add Sub Task</button>
            {selectedTodoId === todo.id && (
              <div>
                ...
                />
                <button onClick={handleAddSubTask} className='Add__todo'>Add</button>
              </div>
            )}
            {todo.subTasks && (
              <ul style={{fontSize:'20px',fontWeight:'bolder'}}>
